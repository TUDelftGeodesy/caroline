...
base_stack = open(f"{caroline_dir}/caroline_v{version}/files/doris_v5/doris_input.xml")
...
for track in range(len(tracks)):
    stack_dir = f"{doris_dir}/{AoI_name}_s1_{asc_dsc[track]}_t{tracks[track]:0>3d}/good_images/20*"
    ...
    doris_stack = stack.format(
        ...
        do_unwrap=do_unwrap,
        start_date=f"{act_start_date[:4]}-{act_start_date[4:6]}-{act_start_date[6:]}",
        end_date=f"{act_end_date[:4]}-{act_end_date[4:6]}-{act_end_date[6:]}",
        master_date=f"{act_master_date[:4]}-{act_master_date[4:6]}-{act_master_date[6:]}",
    )

    fw = open(
        "{}/{}_s1_{}_t{:0>3d}/doris_input.xml".format(
