...
for track in range(len(tracks)):
    if out_parameters["sensor"] == "S1":
        fr = open(
            "{crop_dir}/{AoI_name}_s1_{asc_dsc}_t{fill_track}/cropped_stack/nlines_crp.txt".format(
                crop_dir=out_parameters["crop_directory"],
        ...
        fr = open(
            "{coregistration_dir}/{AoI_name}_{sensor}_{asc_dsc}_t{fill_track}/process/input.resample".format(
    ...
    f = open(
        "{}/{}_{}_{}_t{:0>3d}/psi/nlines_crop.txt".format(
    ...
    f = open(
        "{}/{}_{}_{}_t{:0>3d}/psi/project_id.txt".format(
            out_parameters["depsi_directory"], AoI_name, out_parameters["sensor"].lower(), asc_dsc[track], tracks[track]
        ),
        "w",
    )
