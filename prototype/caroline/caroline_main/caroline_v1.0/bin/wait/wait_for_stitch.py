...
for track in range(len(tracks)):
    jf = open("{}/{}_s1_{}_t{:0>3d}/job_id.txt".format(out_parameters['stitch_directory'], AoI_name, asc_dsc[track],
                                                       tracks[track]))
    job = jf.read().strip()
...
while not finished:
    ...
    for track in range(len(tracks)):
        f = open("{}/{}_s1_{}_t{:0>3d}/dir_contents.txt".format(out_parameters['stitch_directory'], AoI_name, asc_dsc[track], tracks[track]))
        orig_content = f.read().split("\n")
        f.close()
        cur_content = glob.glob("{}/{}_s1_{}_t{:0>3d}/*".format(out_parameters['stitch_directory'], AoI_name, asc_dsc[track], tracks[track]))
        ...
        if len(crop_dir) == 1:
            crop_dir = "{}/{}_s1_{}_t{:0>3d}/{}/*".format(out_parameters['stitch_directory'], AoI_name, asc_dsc[track], tracks[track], crop_dir[0])
            crop_content = glob.glob(crop_dir)
            crop_content = [c.split("/")[-1] for c in crop_content]

        os.system("squeue > {}/{}_s1_{}_t{:0>3d}/queue.txt".format(out_parameters['stitch_directory'], AoI_name, asc_dsc[track], tracks[track]))
        qf = open("{}/{}_s1_{}_t{:0>3d}/queue.txt".format(out_parameters['stitch_directory'], AoI_name, asc_dsc[track], tracks[track]))
        queue = qf.read().split("\n")
        qf.close()
        found = False
