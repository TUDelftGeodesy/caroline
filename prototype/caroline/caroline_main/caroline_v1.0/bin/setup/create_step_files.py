...
for param in out_parameters.keys():
    if param == "depsi_post_mode":
        ...
        if out_parameters[param] == "csv":
            ...
            raise ValueError(f"Unknown depsi post mode {out_parameters[param]}, known are 'csv' and 'tarball'")
    elif param == "dem_file":
        ...
        param == "depsi_AoI_name"
        and len(out_parameters["depsi_AoI_name"]) > 22
        and out_parameters["do_depsi_post"] == 1
    ):
        # This follows from #28
        print(
            f'WARNING: portal upload requested but {out_parameters["depsi_AoI_name"]} is '
...
for prefix in ["doris", "deinsar", "crop", "depsi", "reslc"]:
    ...
    f = open(f"{cpath}/{auxiliary_files}/loop_directories_{prefix}.txt", "w")
