...
for track in range(len(tracks)):
    jf = open("{}/{}_{}_{}_t{:0>3d}/job_id.txt".format(out_parameters['crop_directory'], AoI_name,
...
while not finished:
    now = datetime.now()
    print("\nCrop: Checking on {}-{:0>2d}-{:0>2d} {:0>2d}:{:0>2d}:{:0>2d}".format(now.year,now.month,now.day,now.hour,now.minute,now.second))
    track_status = [False for track in tracks]
    proper_finish = [False for track in tracks]
    for track in range(len(tracks)):
        f = open("{}/{}_{}_{}_t{:0>3d}/dir_contents.txt".format(out_parameters['crop_directory'], AoI_name,
                                                                out_parameters['sensor'].lower(), asc_dsc[track],
                                                                tracks[track]))
        orig_content = f.read().split("\n")
        f.close()
        cur_content = glob.glob("{}/{}_{}_{}_t{:0>3d}/*".format(out_parameters['crop_directory'], AoI_name,
        ...
        if len(crop_dir) == 1:
            crop_dir = "{}/{}_{}_{}_t{:0>3d}/{}/*".format(out_parameters['crop_directory'], AoI_name,
                                                          out_parameters['sensor'].lower(), asc_dsc[track],
                                                          tracks[track], crop_dir[0])
            crop_content = glob.glob(crop_dir)
            crop_content = [c.split("/")[-1] for c in crop_content]

        os.system("squeue > {}/{}_{}_{}_t{:0>3d}/queue.txt".format(out_parameters['crop_directory'], AoI_name,
                                                                   out_parameters['sensor'].lower(), asc_dsc[track],
                                                                   tracks[track]))
        qf = open("{}/{}_{}_{}_t{:0>3d}/queue.txt".format(out_parameters['crop_directory'], AoI_name,
                                                          out_parameters['sensor'].lower(), asc_dsc[track],
                                                          tracks[track]))
        queue = qf.read().split("\n")
        qf.close()
        ...
        if len(crop_dir) >= 1:
            ...
            if len(data) >= 1:
                print("Track {:0>3d} ({}, job_id {})) current last directory: {} ({} done)".format(
                    tracks[track], "Finished" if track_status[track] else "Unfinished",
                ...
                print("Track {:0>3d} ({}, job_id {})) current last directory: N/A (0 done)".format(
                    tracks[track],
