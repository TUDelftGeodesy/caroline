...
for track in range(len(tracks)):	
    try:
        mkdir("{}/{}_{}_{}_t{:0>3d}".format(out_parameters['crop_directory'], AoI_name,
                                            out_parameters['sensor'].lower(),
                                            asc_dsc[track], tracks[track]))
    except OSError:
        pass  # Directory already exists
    f = open("{}/{}_{}_{}_t{:0>3d}/link_directory.txt".format(out_parameters['crop_directory'], AoI_name,
                                                              out_parameters['sensor'].lower(),
                                                              asc_dsc[track], tracks[track]), "w")
    f.write("{}/{}_{}_{}_t{:0>3d}".format(out_parameters['coregistration_directory'], doris_AoI_name,
