...
4. Download the latest `AoI_Summary_XXX.kml` from https://public.spider.surfsara.nl/project/caroline/caroline/caroline-aoi-extents and open it in Google Earth, to check if original SLCs and coregistered stacks already exist in your AoI. If they do, make sure to use these stacks.
5. Modify the parameters in the parameter file to your needs, see [parameter-file.md](parameter-file.md) for an explanation on all parameters. Pay special attention to:
