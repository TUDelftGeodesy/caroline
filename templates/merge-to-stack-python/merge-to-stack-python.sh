...
echo "$(date '+%Y-%m-%dT%H:%M:%S'): $(whoami) in $(pwd) has finished merge-to-stack-python.sh (AoI **merge_to_stack_python:general:AoI-name**, track **track**) with slurm-ID $SLURM_JOB_ID)" >> **caroline_work_directory**/submitted_jobs.log
