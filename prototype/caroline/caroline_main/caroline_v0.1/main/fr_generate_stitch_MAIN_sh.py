...
base_stack = open("/home/sanvandiepen/code/Caroline/Caroline_main/Caroline_v{}/files/stack_stitching/MAIN.sh".format(version))
...
for track in range(len(tracks)):
    ...
    main = stack.format(AoI_name=AoI_name, stitch_dir=out_parameters[0], asc_dsc=asc_dsc[track],track="{:0>3d}".format(tracks[track]),node=node)

    fw = open("{}/{}_s1_{}_t{:0>3d}/MAIN.sh".format(out_parameters[0], AoI_name, asc_dsc[track], tracks[track]), 'w')
    fw.write(main)
    fw.close()
