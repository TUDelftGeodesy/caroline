...
for file in ["input.comprefdem", "input.dembased"]:
    base_stack = open(f"{caroline_dir}/caroline_v{version}/files/doris_v5/input_files/{file}")
    ...
    for track in range(len(tracks)):
        ...
        fw = open(
            "{}/{}_s1_{}_t{:0>3d}/input_files/{}".format(
                out_parameters["coregistration_directory"], AoI_name, asc_dsc[track], tracks[track], file
            ),
            "w",
        )
