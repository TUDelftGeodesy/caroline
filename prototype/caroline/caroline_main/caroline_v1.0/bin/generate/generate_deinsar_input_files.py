...
def get_pixelsize(sensor: str) -> tuple[int, int]:
...
for track in range(len(tracks)):
    basedir = "{}/{}_{}_{}_t{:0>3d}/process".format(
    ...
    for file in [
        ...
        fr = open(f"{caroline_dir}/caroline_v{version}/files/deinsar/input_files/{file}")
        data = fr.read()
        fr.close()
    ...
    for file in ["input.resample"]:
        ...
        for pol in polarisation:
            if pol == "HH":
                data = data_base.format(
                    pol="", center_lat=center_lat, center_lon=center_lon, pix_az=pix_daz, pix_r=pix_dr
                )
                fw = open(f"{basedir}/{file}", "w")
            else:
                data = data_base.format(
                    pol="_" + pol, center_lat=center_lat, center_lon=center_lon, pix_az=pix_daz, pix_r=pix_dr
                )
                fw = open(f"{basedir}/{file}_{pol}", "w")
            fw.write(data)
            fw.close()
        ...
        if out_parameters["sensor"] != "RSAT2":
            ...
            for pol in polarisation:
                if pol == "HH":
                    ...
                    data = data_base.format(
                        img_name=img_name.format(pol="_" + pol),
                        pol="_" + pol,
                        center_lat=center_lat,
                        center_lon=center_lon,
                        pix_az=pix_daz + 500,
    ...
    for file in ["input.comprefdem", "input.dembased", "input.simamp"]:
        ...
        data_base = fr.read()
        data = data_base.format(
            dem_file=out_parameters["dem_file"],
    ...
    for file in ["input.readfiles"]:
        ...
        strng = ""
        for param in order:
            line = "{: <16s}{}\n".format(f"S_IN_{param.upper()}", eval(param))
            strng += line
