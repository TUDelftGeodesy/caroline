...
base_stack = open("{}/caroline_v{}/files/crop/crop.m".format(caroline_dir, version))
...
for track in range(len(tracks)):
    ...
    main = stack.format(shape_dir=out_parameters['shape_directory'],
                        ...
                        version=version, sensor=out_parameters['sensor'])

    fw = open("{}/{}_{}_{}_t{:0>3d}/crop.m".format(out_parameters['crop_directory'], AoI_name,
                                                   out_parameters['sensor'].lower(), asc_dsc[track], tracks[track]), 'w')
    fw.write(main)
    fw.close()
