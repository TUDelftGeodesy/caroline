...
module load **python3_module** **gdal_module** **snap_module**
source ~/.bashrc
...
if [ "${#graph}" -ne "0" ]; then  # the graph actually exists, so we process it
  ...
  echo "No graph to process found with task ID ${SLURM_ARRAY_TASK_ID}, exiting..."
fi
